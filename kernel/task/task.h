#ifndef LEARNOS_TASK_H
#define LEARNOS_TASK_H
...
extern unsigned long _stack_start; //在head.S中定义了

extern void ret_from_intr();

// 任务的状态:运行,可中断,不可中断,僵尸,停止
...
struct task_struct {
    struct List list;
    volatile long state; // 任务的状态
    unsigned long flags; // 任务的种类: 进程,线程,内核线程
    struct mm_struct *mm; // 任务的内存空间分布结构体
    struct thread_struct *thread; // 任务切换相关的信息
    unsigned long addr_limit;    /*0x0000,0000,0000,0000 - 0x0000,7fff,ffff,ffff user; 0xffff,8000,0000,0000 - 0xffff,ffff,ffff,ffff kernel*/
    long pid; //任务号
    long counter; //时间片
    long signal; //信号
    long priority; //优先级
