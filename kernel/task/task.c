...
{
    color_printk(WHITE,BLACK,"__switch_to\n");
...
void switch_to_(struct task_struct *p){
    switch_to(current,p);
}
...
void task_init()
{
    ...
    color_printk(WHITE,BLACK,"%#018lx switch_to:%#018lx\n",(unsigned long)current, (unsigned long)switch_to_);
    color_printk(WHITE,BLACK,"current.thread:%#018lx\n",current->thread);
    switch_to_(p);
}
