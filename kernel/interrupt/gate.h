#ifndef LEARNOS_GATE_H
#define LEARNOS_GATE_H


// 代码段，数据段描述符是一个64位的数据, 就是char[8]
struct desc_struct
{
    unsigned char x[8];
};

// 中断门描述符是一个128位的数据, 这里用char[16]
struct gate_struct
{
    unsigned char x[16];
};

// GDT表,IDT表,TSS表在 head.S 中已经定义了, 所以这里用extern
